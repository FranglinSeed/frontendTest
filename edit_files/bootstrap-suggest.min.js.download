function getMentions(input)
{
	return input.match(/(?:^|[ ])@([a-z.A-Z.0-9]+)/gm);
}
...
/*						case 32: // ?
						case 191: // ?
							...
							{
								fields += "<option value='" + that.$items.get(i).dataset.value + "'>" + that.$items.get(i).innerHTML + "</option>";
							}
							
									var html = '<b>Select </b><select name="_selc" id="_selc">' + fields + '</option></select>';
									var statesdemo = {
										state0: {
											html:html,
											...
											submit:function(e,v,m,f){ 
											    if ( v )
												{
													...
													el.value = ( oldVal.substring(0, setCaretPos) + f._selc + oldVal.substring(setCaretPos+1) );
													that.hide();
												}
											}// end of submit
